...
public class ReadingAdapter {
	...
	/**
	 * Create a new reading If the reading is successfully created return the
	 * new rowId for that note, otherwise return a -1 to indicate failure.
	 */

	public long createReading(Long traverse, Integer observation, Integer type,
			Double reading, String label, String modified_date) {
		ContentValues values = createContentValues(traverse, observation, type,
				reading, label, modified_date);

		return db.insert(DB_TABLE, null, values);
	}

	/**
	 * Update the reading, returns true or false based on whether the update was
	 * sucessful
	 */

	public boolean updateReading(long rowId, Long traverse,
			Integer observation, Integer type, Double reading, String label,
	...
	/**
	 * Deletes reading, returns true or false based on whether the update was
	 * successful
	 */
	...
	public Cursor fetchObservationReadings(Long traverse) {
		return db.query(DB_TABLE, new String[] { KEY_ROWID, KEY_TRAVERSE,
				KEY_OBSERVATION, KEY_TYPE, KEY_READING, KEY_LABEL,
				KEY_MODIFIEDDATE }, KEY_TRAVERSE + "=" + traverse, null, null,
				null, null, null);

	}

	/**
	 * Return a Cursor over the list of all readings that have already been
	 * taken for the IS
	 * 
	 * @return Cursor over all readings
	 */
	...
	public Cursor fetchISReadings(Long traverse, Integer observation,
		...
		return db.query(DB_TABLE, new String[] { KEY_ROWID, KEY_TRAVERSE,
				KEY_OBSERVATION, KEY_TYPE, KEY_READING, KEY_LABEL,
				KEY_MODIFIEDDATE }, KEY_TRAVERSE + "=" + traverse + " AND "
				+ KEY_OBSERVATION + "=" + observation + " AND " + KEY_TYPE
	...
	public Cursor fetchAllReadings() {
		return db.query(DB_TABLE, new String[] { KEY_ROWID, KEY_TRAVERSE,
				KEY_OBSERVATION, KEY_TYPE, KEY_READING, KEY_LABEL,
				KEY_MODIFIEDDATE }, null, null, null, null, null);
	}

	/**
	 * Check if there is already a reading taken of the given type at the
	 * observation point
	 */

	public boolean checkDuplicate(Long traverse, Integer observation,
			Integer type) throws SQLException {
		Cursor mCursor = db.query(DB_TABLE, new String[] { KEY_ROWID,
				KEY_TRAVERSE, KEY_OBSERVATION, KEY_TYPE, KEY_READING,
				KEY_LABEL, KEY_MODIFIEDDATE }, KEY_TRAVERSE + "=" + traverse
				+ " AND " + KEY_OBSERVATION + "=" + observation + " AND "
	...
	public Integer nextObservation(Long traverse) throws SQLException {
		...
		if (mCursor.moveToFirst() != false) {
			...
			for (i = 0; i < mCursor.getCount(); i++) {
				...
				if (d != mCursor.getInt(2) && d <= mCursor.getInt(2)) {
					d = mCursor.getInt(2);
				}
	...
	/**
	 * Return the id for of the reading that has already been taken matching the
	 * type and observation
	 */

	public long idDuplicate(Long traverse, Integer observation, Integer type)
			throws SQLException {
		Cursor mCursor = db.query(DB_TABLE, new String[] { KEY_ROWID,
				KEY_TRAVERSE, KEY_OBSERVATION, KEY_TYPE, KEY_READING,
				KEY_LABEL, KEY_MODIFIEDDATE }, KEY_TRAVERSE + "=" + traverse
	...
	public Cursor fetchReading(long rowId) throws SQLException {
		Cursor mCursor = db.query(DB_TABLE, new String[] { KEY_ROWID,
				KEY_TRAVERSE, KEY_OBSERVATION, KEY_TYPE, KEY_READING,
				KEY_LABEL, KEY_MODIFIEDDATE }, KEY_ROWID + "=" + rowId, null,
				null, null, null, null);
		if (mCursor != null) {
			mCursor.moveToFirst();
		}
	...
	private ContentValues createContentValues(Long traverse,
			Integer observation, Integer type, Double reading, String label,
			String modified_date) {
